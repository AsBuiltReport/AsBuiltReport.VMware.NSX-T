function Get-AbrNsxtQosProfiles {
    ...
    process {
        $QosProfiles = get-abrNsxtApi -uri "/policy/api/v1/infra/qos-profiles"
        if($QosProfiles.result_count -ge 1){
            $QosProfileInfo = foreach ($QosProfile in $QosProfiles.results){
            ...
            Section -Style Heading4 "QoS Profiles" {
                Paragraph 'No Qos Profiles Configured'
            }
        }
