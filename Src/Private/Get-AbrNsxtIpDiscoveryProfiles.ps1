function Get-AbrNsxtIpDiscoveryProfiles {
    ...
    process {
        $IpDiscoveryProfiles= get-abrNsxtApi -uri "/policy/api/v1/infra/ip-discovery-profiles"
        If($IpDiscoveryProfiles.result_count -ge 1){
            $IpDiscoveryProfileInfo = foreach ($IpDiscoveryProfile in $IpDiscoveryProfiles.results){
            ...
            Section -Style Heading4 "IP Discovery Profiles" {
                Paragraph 'No IP Discovery Profiles Configured'
            }
        }
    }
