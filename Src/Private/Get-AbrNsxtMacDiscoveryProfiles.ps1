function Get-AbrNsxtMacDiscoveryProfiles {
    ...
    process {
        $MacDiscoveryProfiles= get-abrNsxtApi -uri "/policy/api/v1/infra/mac-discovery-profiles"
        If($MacDiscoveryProfiles.result_count -ge 1){
            $MacDiscoveryProfileInfo = foreach ($MacDiscoveryProfile in $MacDiscoveryProfiles.results){
            ...
            Section -Style Heading4 "MAC Discovery Profiles" {
                Paragraph 'No MAC Discovery Profiles Configured'
            }
        }
    }
